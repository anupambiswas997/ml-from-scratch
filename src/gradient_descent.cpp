...
IGradientDescentSolver::IGradientDescentSolver(const Matrix& X, const Vector& y, size_t numIterations, double tolerance)
{
    m_pX = &X;
    m_py = &y;
...
void IGradientDescentSolver::solve()
{
    // Initialize bias and weights.
    m_bias = getRandom();
    m_weights = getRandomVector(m_pX->getNumColumns());
